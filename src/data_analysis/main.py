"""Main entry point for the multi-agent data analysis application."""


import os
import warnings
from data_analysis.crew import DataAnalysis
...
def run():
    """Run the multi-agent crew for data analysis."""
    ...
    # Extract and save schema metadata BEFORE launching agents
    update_analysis_status("raw_schema", "in_progress")
    extract_and_save_schema_metadata()
    update_analysis_status("raw_schema", "done")
    print("✅ Metadata Extraction")
    
    # Update status: schema_interpreter starts
    update_analysis_status("schema_interpreter", "in_progress")

    # Launch the crew (no inputs needed as all values are hardcoded in YAML)
    DataAnalysis().crew().kickoff(inputs={})
