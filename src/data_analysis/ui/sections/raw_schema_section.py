"""Display section for raw schema metadata (data sent to agents)."""
...
def display_raw_schema(data: dict):
    """Display the raw schema metadata extracted from CSV files.
    
    Args:
        data: Dictionary containing the raw schema metadata.
    """
    ...
    # --------------------------------------------------------------------
    with main_tabs[0]:
        st.subheader("Raw metadata extracted from CSV files")
        ...
        summary_rows = [
            {
                ...
                "Primary key": info.get("primary_key", "N/A") or "None",
    ...
    # --------------------------------------------------------------------
    with main_tabs[2]:
        st.subheader("Detected relations between tables")
