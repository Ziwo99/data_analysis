"""Display section for query builder results with code and DataFrames."""
...
def display_queries_analysis(data: dict):
    """Display query results with code and DataFrame tabs for each sub-analysis.
    
    Args:
        data: Dictionary containing the query analysis results.
    """
    ...
    if not analyses:
        st.info("No query analysis available.")
        return
            ...
            if not sub_analyses:
                ...
                for sub_analysis in sub_analyses:
                    ...
                    with st.expander(f"{sub_id} - {sub_title}", expanded=False):
                        ...
                        # --------------------------------------------------------------------
                        # TWO TABS : CODE PYTHON + DATAFRAME
                        # --------------------------------------------------------------------
                        if code_lines or result_dataframe is not None or query_error:
                            tab_dataframe, tab_code = st.tabs(["Dataframe", "Query code"])

                            # TAB 1 - Result DataFrame
                            with tab_dataframe:
                            ...
                            with tab_code:
                                if code_lines:
                                    code_str = "\n".join(code_lines)
                                    # Use st.code() for syntax highlighting
