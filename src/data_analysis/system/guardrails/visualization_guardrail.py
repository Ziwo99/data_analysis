"""Guardrail for validating and executing visualization designer output."""
...
def visualization_guardrail(result: TaskOutput) -> Tuple[bool, Any]:
    """Validate visualization output, execute code, and save results.
    ...
    try:
        ...
        attempts = get_agent_attempts("visualization_designer")
        MAX_ATTEMPTS = 4
        
        # Check BEFORE incrementing to prevent going over max
        if attempts >= MAX_ATTEMPTS:
            update_analysis_status("visualization_designer", "error")
            mark_following_agents_as_error("visualization_designer", mono_mode=False)
            print(f"❌ Visualization Designer - Failed after {MAX_ATTEMPTS} attempts")
            return (False, format_json_error(e))
        
        # Only increment if we haven't reached max yet
        increment_agent_attempts("visualization_designer")
        attempts = get_agent_attempts("visualization_designer")
        # Cap attempts display at max to prevent display issues
        display_attempts = min(attempts, MAX_ATTEMPTS)
        print(f"❌ Visualization Designer - Retry (attempt {display_attempts}/{MAX_ATTEMPTS})")
        return (False, format_json_error(e))
    ...
    # Check for success
    if not failed_items:
        update_analysis_status("visualization_designer", "done")
        print("✅ Visualization Designer")
        update_analysis_status("confidentiality_tester", "in_progress")
    ...
    increment_agent_attempts("visualization_designer")
    attempts = get_agent_attempts("visualization_designer")
    # Cap attempts display at max to prevent display issues (5/4)
    display_attempts = min(attempts, MAX_ATTEMPTS)
    print(f"❌ Visualization Designer - Retry (attempt {display_attempts}/{MAX_ATTEMPTS})")
    return (False, format_visualization_errors(failed_items))